...
static u8 PokeSum_EffectiveFlipDirection(void);
static u8 PokeSum_GetOldPage(void);
...
static void BufferMonAbilities(void);
static u8 StatusToAilment(u32 status);
static void BufferMonMoveI(u8);
static void BufferMonAbilityI(u8);
static u16 GetMonMoveBySlotId(struct Pokemon * mon, u8 moveSlot);
static u16 GetMonAbilityBySlotId(struct Pokemon * mon, u8 abilitySlot);
static u16 GetMonPpByMoveSlot(struct Pokemon * mon, u8 moveSlot);
static u16 GetMonApByAbilitySlot(struct Pokemon * mon, u8 abilitySlot);
...
static void PokeSum_PrintAbilityName(u8 i);
...
struct PokemonSummaryScreenData
{
    ...
    struct PokeSummary
    {
        ...
        u8 ALIGNED(4) abilityApStrBufs[5][11];
        u8 ALIGNED(4) abilityNameStrBufs[5][ABILITY_NAME_LENGTH + 1];
    ...
    u8 ALIGNED(4) numAbilities;
    // TODO: add isSwappingAbilities
...
extern const u32 gBgTilemap_PokeSum_AbilitiesListForDelete[];
...
static const u8 sPrintAbilityTextColors[][3] = {
    {0, 7, 8},
};
...
#define POKESUM_WIN_ABILITIES_3      3
#define POKESUM_WIN_ABILITIES_4      4
#define POKESUM_WIN_ABILITIES_5      5
#define POKESUM_WIN_ABILITIES_6      6
...
static const struct WindowTemplate sWindowTemplates_Abilities[] =
{
    [POKESUM_WIN_ABILITIES_3 - 3] = {
        .bg = 0,
        .tilemapLeft = 17,
        .tilemapTop = 2,
        .width = 13,
...
void ShowPokemonSummaryScreen(struct Pokemon * party, u8 cursorPos, u8 lastIdx, MainCallback savedCallback, u8 mode)
{
    ...
    case PSS_MODE_FORGET_MOVE:
        ...
        break;
    // TODO: add case for mode select ability / learn ability
    }
    ...
    sMonSummaryScreen->lastPageFlipDirection = PSS_FLIP_OFF;
...
bool32 IsPageFlipInput(u8 direction)
{
    ...
    if (sMonSummaryScreen->lastPageFlipDirection != PSS_FLIP_OFF && sMonSummaryScreen->lastPageFlipDirection == direction)
    {
        sMonSummaryScreen->lastPageFlipDirection = PSS_FLIP_OFF;
        return TRUE;
    }
...
static void Task_InputHandler_Info(u8 taskId)
{
    ...
    case PSS_STATE3270_HANDLEINPUT:
        ...
        {
            if (IsPageFlipInput(PSS_FLIP_RIGHT) == TRUE)
            {
                if (FuncIsActiveTask(Task_PokeSum_FlipPages))
                {
                    sMonSummaryScreen->lastPageFlipDirection = PSS_FLIP_RIGHT;
                    return;
                }
                else if (sMonSummaryScreen->curPageIndex < PSS_PAGE_ABILITIES)
                {
                    PlaySE(SE_SELECT);
                    HideBg(0);
                    sMonSummaryScreen->pageFlipDirection = PSS_FLIP_RIGHT;
            ...
            else if (IsPageFlipInput(PSS_FLIP_LEFT) == TRUE)
            {
                if (FuncIsActiveTask(Task_PokeSum_FlipPages))
                {
                    sMonSummaryScreen->lastPageFlipDirection = PSS_FLIP_LEFT;
                    return;
                }
                else if (sMonSummaryScreen->curPageIndex > PSS_PAGE_INFO)
                {
                    PlaySE(SE_SELECT);
                    HideBg(0);
                    sMonSummaryScreen->pageFlipDirection = PSS_FLIP_LEFT;
                    PokeSum_RemoveWindows(sMonSummaryScreen->curPageIndex);
                    sMonSummaryScreen->curPageIndex--;
                    sMonSummaryScreen->state3270 = PSS_STATE3270_FLIPPAGES;
                }
        ...
        if ((!FuncIsActiveTask(Task_PokeSum_FlipPages)) || FuncIsActiveTask(Task_PokeSum_SwitchDisplayedPokemon))
        {
            ...
            else if (JOY_NEW(A_BUTTON))
            {
                ...
                {
                    PlaySE(SE_SELECT);
                    sMonSummaryScreen->pageFlipDirection = PSS_FLIP_OUT;
                    PokeSum_RemoveWindows(sMonSummaryScreen->curPageIndex);
                    sMonSummaryScreen->curPageIndex = PSS_PAGE_MOVES_INFO;
                    sMonSummaryScreen->state3270 = PSS_STATE3270_FLIPPAGES;
                }
                // TODO: add case for abilities
                return;
            }
...
static void Task_PokeSum_FlipPages(u8 taskId)
{
    ...
    case 0:
        ...
        break;
    case 1:
        if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_LEFT || sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT)
        {
...
static u8 PokeSum_EffectiveFlipDirection(void)
{
    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_LEFT || sMonSummaryScreen->pageFlipDirection == PSS_FLIP_IN)
        return PSS_FLIP_LEFT;
    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT || sMonSummaryScreen->pageFlipDirection == PSS_FLIP_OUT)
        return PSS_FLIP_RIGHT;
    return PSS_FLIP_OFF;
}

static u8 PokeSum_GetOldPage(void)
{
    switch (sMonSummaryScreen->pageFlipDirection)
    {
        case PSS_FLIP_RIGHT:
...
static void PokeSum_InitBgCoordsBeforePageFlips(void)
{
    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_OUT)
    {
        sMonSummaryScreen->flipPagesBgHofs = 240;
        return;
    }

    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_IN)
    {
    ...
    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT)
    {
        sMonSummaryScreen->flipPagesBgHofs = 0;
    ...
    else // sMonSummaryScreen->pageFlipDirection == PSS_FLIP_LEFT
    {
    ...
    else if (sMonSummaryScreen->curPageIndex == PSS_PAGE_MOVES && sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT)
        PokeSum_SetHpExpBarCoordsFullLeft();
...
static void PokeSum_ShowSpritesBeforePageFlip(void)
{
    ...
    case PSS_PAGE_SKILLS:
        break;
    case PSS_PAGE_MOVES:
        if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_LEFT)
        {
            ShowOrHideHpBarObjs(FALSE);
            ShowOrHideExpBarObjs(FALSE);
        }
        // TODO: case for direction 1 or 2
        else if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_OUT)
        {
...
static void PokeSum_UpdateBgPriorityForPageFlip(u8 setBg0Priority, u8 keepBg1Bg2PriorityOrder)
{
    ...
    if (PokeSum_EffectiveFlipDirection() == PSS_FLIP_RIGHT)
    {
        if (setBg0Priority == 0)
        {
            bg0Priority = 0;
    ...
    if (PokeSum_EffectiveFlipDirection() == PSS_FLIP_LEFT)
    {
        bg0Priority = 0;
        if (bg1Priority > bg2Priority)
            bg1Priority = 1, bg2Priority = 2;
    ...
    for (i = 0; i < 11; i++)
    {
        if (sMonSummaryScreen->curPageIndex == PSS_PAGE_SKILLS && sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT)
...
static void PokeSum_CopyNewBgTilemapBeforePageFlip_2(void)
{
    ...
    case PSS_PAGE_MOVES:
        ...
        else if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT)
            CopyToBgTilemapBuffer(sMonSummaryScreen->infoAndMovesPageBgNum, gBgTilemap_PokeSum_AbilitiesListForDelete, 0, 0);
        else
            CopyToBgTilemapBuffer(sMonSummaryScreen->infoAndMovesPageBgNum, gBgTilemap_TrainerMemo_Stats, 0, 0);
...
static void PokeSum_CopyNewBgTilemapBeforePageFlip(void)
{
    ...
    case PSS_PAGE_SKILLS:
        break;
    case PSS_PAGE_MOVES:
        // TODO: cases for 1 and 2
        if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_OUT)
            CopyToBgTilemapBuffer(3, sBgTilemap_MovesPage, 0, 0);
        else if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_LEFT)
...
static void BufferMonAbilities(void)
{
    ...
    // TODO: handle mode_select_ability
    // if (sMonSummaryScreen->mode == PSS_MODE_SELECT_MOVE)
    //     BufferMonMoveI(4);
}

static void BufferMonAbilityI(u8 i)
{
    if (i < 4)
        sMonSummaryScreen->abilityIds[i] = GetMonAbilityBySlotId(&sMonSummaryScreen->currentMon, i);

    if (sMonSummaryScreen->abilityIds[i] == 0)
    {
        StringCopy(sMonSummaryScreen->summary.abilityNameStrBufs[i], gText_PokeSum_OneHyphen);
        StringCopy(sMonSummaryScreen->summary.abilityApStrBufs[i], gText_PokeSum_TwoHyphens);
        sMonSkillsPrinterXpos->ap[i] = 0xff;
        return;
    }

    sMonSummaryScreen->numAbilities++;
    StringCopy(sMonSummaryScreen->summary.abilityNameStrBufs[i], gAbilityNames[sMonSummaryScreen->abilityIds[i]]);

    // TODO: add case for ability select mode
    // if (i >= 4 && sMonSummaryScreen->mode == PSS_MODE_SELECT_MOVE)
    if (0) {}
    else
    {
        ConvertIntToDecimalStringN(sMonSummaryScreen->summary.abilityApStrBufs[i],
                                   GetMonApByAbilitySlot(&sMonSummaryScreen->currentMon, i), STR_CONV_MODE_LEFT_ALIGN, 3);
    }

    sMonSkillsPrinterXpos->ap[i] = GetRightAlignXpos_NDigits(2, sMonSummaryScreen->summary.abilityApStrBufs[i]);
}
...
#define GetAbilityNamePrinterYpos(y) ((y) * 28 + 5)
#define GetAbilityApPrinterYpos(y) ((y) * 28 + 16)
...
static void PrintAbilitiesPage(void)
{
    ...
    for (i = 0; i < 4; i++)
        PokeSum_PrintAbilityName(i);

    // TODO: implement for page abilities info
    // if (sMonSummaryScreen->curPageIndex == PSS_PAGE_MOVES_INFO)
    // {
    //     if (sMonSummaryScreen->mode == PSS_MODE_SELECT_MOVE)
    //         PokeSum_PrintMoveName(4);
...
static void PokeSum_PrintAbilityName(u8 i)
{
    ...
    AddTextPrinterParameterized3(
        ...
        2,
        3,
        GetAbilityNamePrinterYpos(i),
        sPrintAbilityTextColors[0],
        TEXT_SPEED_FF,
        sMonSummaryScreen->summary.abilityNameStrBufs[i]
    );
    ...
    AddTextPrinterParameterized3(
        ...
        2,
        60,
        GetAbilityApPrinterYpos(i),
        sPrintAbilityTextColors[colorIdx],
        TEXT_SPEED_FF,
        gText_PokeSum_AP
    );
    ...
    AddTextPrinterParameterized3(
        sMonSummaryScreen->windowIds[POKESUM_WIN_RIGHT_PANE],
        2,
        80 + sMonSkillsPrinterXpos->ap[i],
        GetAbilityApPrinterYpos(i),
        sPrintMoveTextColors[colorIdx],
        TEXT_SPEED_FF,
        sMonSummaryScreen->summary.abilityApStrBufs[i]
    );
}
...
static void PokeSum_PrintPageHeaderText(u8 curPageIndex)
{
    ...
    case PSS_PAGE_MOVES:
        ...
        break;
    case PSS_PAGE_ABILITIES:
        PokeSum_PrintPageName(gText_PokeSum_PageName_EquippedAbilities);
        PokeSum_PrintControlsString(gText_PokeSum_Controls_PageDetail2);
        PrintMonLevelNickOnWindow2(gText_PokeSum_NoData);
        break;
    case PSS_PAGE_MOVES_INFO:
        PokeSum_PrintPageName(gText_PokeSum_PageName_KnownMoves);
...
static void PokeSum_FlipPages_SlideLayerRight(void)
{
...
static void PokeSum_FlipPages_HandleBgHofs(void)
{
    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT)
            ...
            PokeSum_FlipPages_SlideLayerRight();
    else if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_OUT)
            PokeSum_FlipPages_SlideLayerRight();
    else if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_IN)
            PokeSum_FlipPages_SlideLayerLeft();
}
...
static void PokeSum_CreateWindows(void)
{
    ...
    for (i = 0; i < 4; i++)
        ...
        case PSS_PAGE_MOVES_INFO:
            ...
            break;
        case PSS_PAGE_ABILITIES:
            sMonSummaryScreen->windowIds[i + 3] = AddWindow(&sWindowTemplates_Abilities[i]);
            break;
        default:
            break;
        }
...
static void PokeSum_AddWindows(u8 curPageIndex)
{
    ...
    if (sMonSummaryScreen->pageFlipDirection == PSS_FLIP_RIGHT || sMonSummaryScreen->pageFlipDirection == PSS_FLIP_IN)
    {
...
static void PokeSum_SetHelpContext(void)
{
    ...
    case PSS_PAGE_MOVES_INFO:
        ...
        break;
    case PSS_PAGE_ABILITIES:
        // TODO: implement help context for pokemon abilities
        SetHelpContext(HELPCONTEXT_POKEMON_MOVES);
        break;
    }
}
...
static u16 GetMonAbilityBySlotId(struct Pokemon * mon, u8 abilitySlot)
{
    ...
    {
    case 0:
        ability = GetMonData(mon, MON_DATA_ABILITY1);
        break;
    case 1:
        ability = GetMonData(mon, MON_DATA_ABILITY2);
        break;
    case 2:
        ability = GetMonData(mon, MON_DATA_ABILITY3);
        break;
    default:
        ability = GetMonData(mon, MON_DATA_ABILITY4);
    }
    ...
    case 0:
        ...
        else if (JOY_NEW(A_BUTTON))
        {
            ...
            if (sMoveSelectionCursorPos == 4)
            {
                ...
                ShoworHideMoveSelectionCursor(TRUE);
                sMonSummaryScreen->pageFlipDirection = PSS_FLIP_IN;
                PokeSum_RemoveWindows(sMonSummaryScreen->curPageIndex);
